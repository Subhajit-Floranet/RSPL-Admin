...
use App\Models\GBGOrder;
use App\Models\GBSOrder;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use Auth;


class OrderManagementController extends CommonController
{
    /**
    ...
    public function allorder(Request $request){
        ...
        //$gbg = GBGOrder::where('payment_status', 'C')->select('unique_order_id');
        //$gbs = GBSOrder::where('payment_status', 'C')->select('unique_order_id');

        $gbg = DB::table('gbg.orders')->where('payment_status', 'C')->select('unique_order_id','payment_method','payment_status','order_delivery_status','purchase_date','created_at');
        $gbs = DB::table('gbs.orders')->where('payment_status', 'C')->select('unique_order_id','payment_method','payment_status','order_delivery_status','purchase_date','created_at');
        ...
        if($request->isMethod('POST')){
           ...
           if($val == 1){
            ...
            $gbs = DB::table('gbs.orders')->where('payment_status', 'C')->select('unique_order_id','payment_method','payment_status','order_delivery_status','purchase_date','created_at');
            $res = $gbg->unionAll($gbs)->orderBy('created_at', 'desc')->get();
            return view('admin.Order.allorderlist',["result" => $res, 'request' => $request]);
           }
           elseif($val==2){
            $gbg = GBGOrder::orderBy('created_at', 'desc')->where('payment_status', 'C')->get();
            return view('admin.Order.gbgorderlist',["result" => $gbg, 'request' => $request]);
           }
           else{
            $gbg = GBGOrder::orderBy('created_at', 'desc')->where('payment_status', 'C')->get();
            return view('admin.Order.gbgorderlist',["result" => $gbg, 'request' => $request]);
    ...
    public function gbgorder(Request $request){

        $gbg = GBGOrder::orderBy('created_at', 'desc')->where('payment_status', 'C')->get();
        return view('admin.Order.gbgorderlist',["result" => $gbg, 'request' => $request]);
    }
    
    public function gbsorder(Request $request){

        $gbs = GBSOrder::orderBy('created_at', 'desc')->where('payment_status', 'C')->get();
        return view('admin.Order.gbsorderlist',["result" => $gbs, 'request' => $request]);
    }
    ...
    //     return view('admin.gbg.cms.add', ['request' => $request, 'websiteShortCode' => $websiteShortCode]);
    // }
