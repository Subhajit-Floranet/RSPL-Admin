...
<div class="page-breadcrumb">
    <div class="row">
        <div class="col-12 d-flex no-block align-items-center">
            <h4 class="page-title">{{ strtoupper($websiteShortCode) }} Coupon</h4>
            <div class="ml-auto text-right">
                <a href="{{ route('admin.'.$websiteShortCode.'.coupon.add') }}" class="btn btn-success">ADD Coupon</a>
            </div>
        </div>
    </div>
</div>
                        ...
                        <table id="zero_config" class="table table-striped table-bordered">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 35%"><b>Coupon Code</b></th>
                                    <th style="width: 5%"><b>Discount Type</b></th>
                                    <th style="width: 20%"><b>Discount Amount</b></th>
                                    <th style="width: 6%"><b>Start Date</b></th>
                                    <th style="width: 6%"><b>End Date</b></th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    @if ($data->type == "F") <td>Flat</td>  @else  <td>Percent</td>  @endif
                                    <td>{{$data->amount}}</td>
                                    ...
                                    <td >
                                        <a href="{{ route('admin.'.$websiteShortCode.'.coupon.edit', base64_encode($data->id).'?redirect='.urlencode($request->fullUrl())) }}">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <a onclick="return confirm('Are you sure you want to delete the Coupon?')" href="{{ route('admin.'.$websiteShortCode.'.coupon.delete', base64_encode($data->id)) }}">
...
<script>
    ...
    $(function(){
		...
		$('.blockstatus').change(function(){
			...
			var ajaxurl = "{{ route('admin.'.$websiteShortCode.'.coupon.status') }}";	
			//alert(ajaxurl);
			$.ajax({
				type : 'POST',
				url : ajaxurl,
